...
class Maneuver:
  def __init__(self, title, duration, **kwargs):
    ...
    self.radar_lead = kwargs.get("radar_lead", True)
    self.ensure_start = kwargs.get("ensure_start", False)
    self.ensure_slowdown = kwargs.get("ensure_slowdown", False)
    self.ensure_cruise_speed = kwargs.get("ensure_cruise_speed", False)
  ...
  def evaluate(self):
    ...
    while plant.current_time < self.duration:
      ...
      log['v_rel'] = v_rel
      numeric_outputs = (
        log['distance'], log['speed'], log['acceleration'], log['distance_lead'],
        log['d_rel'], log['v_rel'],
      )
      if not np.all(np.isfinite(numeric_outputs)):
        if not nonfinite_reported:
          print('LongitudinalPlanner produced a non-finite maneuver output!')
    ...
    observed_speeds = np.asarray(logs)[:, 3]
    if self.ensure_cruise_speed and np.any(np.abs(observed_speeds - self.speed) > 0.5):
      print('LongitudinalPlanner unexpectedly deviating from cruise speed!')
      valid = False

    stopping_commanded = log['should_stop'] and log['speed'] <= 0.5
    if self.force_decel and self.enabled and not stopping_commanded and log['speed'] > 1e-1 and log['acceleration'] > -0.04:
