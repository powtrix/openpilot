...
const CanMsg HYUNDAI_CANFD_HDA2_TX_MSGS[] = {
  ...
  {0x362, 0, 32}, // CAM_0x362 can be the cached suppression source
};

const CanMsg HYUNDAI_CANFD_HDA2_ALT_BUTTONS_TX_MSGS[] = {
  {0x50, 0, 16},  // LKAS
  {0x1AA, 1, 16}, // CRUISE_ALT_BUTTONS
  {0x2A4, 0, 24}, // CAM_0x2A4
  {0x362, 0, 32}, // CAM_0x362 can be the cached suppression source
};
...
const CanMsg HYUNDAI_CANFD_HDA2_ALT_STEERING_TX_MSGS[] = {
  ...
  {0x2A4, 0, 24}, // CAM_0x2A4 can be the cached suppression source
};

const CanMsg HYUNDAI_CANFD_HDA2_ALT_STEERING_ALT_BUTTONS_TX_MSGS[] = {
  {0x110, 0, 32}, // LKAS_ALT
  {0x1AA, 1, 16}, // CRUISE_ALT_BUTTONS
  {0x362, 0, 32}, // CAM_0x362
  {0x2A4, 0, 24}, // CAM_0x2A4 can be the cached suppression source
...
// Stock-long HDA1 does not disable the radar/ADAS ECUs. Only the lateral
// messages and the selected cruise-button encoding are generated by
// CarController; keep the broader list above for openpilot-long and the
// existing camera-SCC harness path.
const CanMsg HYUNDAI_CANFD_HDA1_STOCK_TX_MSGS[] = {
  {0x12A, 0, 16}, // LFA
  {0x1CF, 0, 8},  // CRUISE_BUTTON
  {0x1E0, 0, 16}, // LFAHDA_CLUSTER
...
typedef struct {
  int addr;
  int bus;              // transmit bus whose matching forwarded frame is replaced: 0 or 2
  int hz;
  uint32_t timeout_us;
  uint32_t last_tx_us;
...
// Forwarding replacement state is tracked only for buses 0 and 2.
CanfdTxState canfd_tx_states[] = {
  {0x50,  0, 100, 0U, 0U, false, 0U}, // 80:  LKAS
  {0x51,  0, 100, 0U, 0U, false, 0U}, // 81:  ADRV_0x51
  {0x110, 0, 100, 0U, 0U, false, 0U}, // 272: LKAS_ALT
  {0x12A, 0, 100, 0U, 0U, false, 0U}, // 298: LFA
  {0x160, 0, 50,  0U, 0U, false, 0U}, // 352: ADRV_0x160
  {0x161, 0, 20,  0U, 0U, false, 0U}, // 353: ADRV_0x161
  {0x162, 0, 20,  0U, 0U, false, 0U}, // 354: CCNC_0x162
  {0x1A0, 0, 50,  0U, 0U, false, 0U}, // 416: SCC_CONTROL
  {0x1DA, 0, 1,   0U, 0U, false, 0U}, // 474: ADRV_0x1da
  {0x1E0, 0, 20,  0U, 0U, false, 0U}, // 480: LFAHDA_CLUSTER
  {0x1EA, 0, 20,  0U, 0U, false, 0U}, // 490: ADRV_0x1ea
  {0x200, 0, 20,  0U, 0U, false, 0U}, // 512: ADRV_0x200
  {0x2A4, 0, 20,  0U, 0U, false, 0U}, // 676: CAM_0x2a4
  {0x345, 0, 5,   0U, 0U, false, 0U}, // 837: ADRV_0x345
  {0x362, 0, 10,  0U, 0U, false, 0U}, // 866: CAM_0x362
  {0x0CB, 0, 100, 0U, 0U, false, 0U}, // 203: LFA_ALT

  {0x175, 2, 50,  0U, 0U, false, 0U}, // 373: TCS
  {0x1AA, 2, 50,  0U, 0U, false, 0U}, // 426: CRUISE_ALT_BUTTONS
  {0x1CF, 2, 50,  0U, 0U, false, 0U}, // 463: CRUISE_BUTTON
  {0x1FA, 2, 10,  0U, 0U, false, 0U}, // 506: CLUSTER_SPEED_LIMIT
  {0x0EA, 2, 100, 0U, 0U, false, 0U}, // 234: MDPS
  {0x2AF, 2, 10,  0U, 0U, false, 0U}, // 687: STEER_TOUCH_2AF
  {0x4A3, 2, 5,   0U, 0U, false, 0U}, // 1187: HDA_INFO_4A3
  {0x4B4, 2, 10,  0U, 0U, false, 0U}, // 1204: NEW_MSG_4B4
  {0x4BE, 2, 10,  0U, 0U, false, 0U}, // 1214: NEW_MSG_4BE
  {0x4B9, 2, 10,  0U, 0U, false, 0U}, // 1209: NEW_MSG_4B9

  {0, 0, 0, 0U, 0U, false, 0U},
};
...
static bool canfd_is_cruise_button_addr(int addr) {
  return (addr == 0x1AA) || (addr == 0x1CF);
}

static void canfd_record_tx_time(const CANPacket_t *to_send, bool tx) {
  ...
  if (st != NULL) {
    ...
    if (canfd_is_cruise_button_addr(addr)) {
      st->has_last_counter = tx;
      st->last_counter = tx ? hyundai_canfd_get_counter(to_send) : 0U;
    }
  }
}

static bool canfd_should_block_fwd(int tx_bus, const CANPacket_t *to_send, uint32_t now) {
  const int addr = GET_ADDR(to_send);
  ...
  if (canfd_is_cruise_button_addr(addr)) {
    // A host button replaces only the stock frame with the same alive counter.
    // A time-only block can suppress the following NONE/release counter when
    // host transmission occurs a few milliseconds after the 50 Hz source.
    return st->has_last_counter && ((now - st->last_tx_us) < st->timeout_us) &&
           (hyundai_canfd_get_counter(to_send) == st->last_counter);
  }
...
static bool canfd_bfwd_pop(CanfdBufferedFwd* st, CANPacket_t* pkt) {
  ...
  // Save the last successfully emitted packet for bounded reuse.
...
static bool hyundai_canfd_tx_hook(const CANPacket_t *to_send_const) {
  ...
  canfd_record_tx_time(to_send, tx);
...
static int hyundai_canfd_fwd_hook(CANPacket_t* to_send) {
  ...
  if (hyundai_canfd_buffered_fwd) {
    ...
    if (bfwd != NULL) {
      ...
      // Reuse the last successful packet once or twice if the queue is empty.
      if (!use_buffered) {
  ...
  if (bus_num == 0) {
    if (canfd_should_block_fwd(2, to_send, now)) {
      return -1;
    }
    if (addr == 0x4B9) {
      return -1;
  ...
  if (canfd_should_block_fwd(0, to_send, now)) {
    return -1;
  }
  return 0;
}
...
static safety_config hyundai_canfd_init(uint16_t param) {
  ...
  for (int i = 0; canfd_tx_states[i].addr > 0; i++) {
    ...
    canfd_tx_states[i].has_last_counter = false;
    canfd_tx_states[i].last_counter = 0U;
  }
  ...
  if (hyundai_longitudinal) {
    ...
    if (hyundai_canfd_hda2 && hyundai_camera_scc) {
      if (hyundai_canfd_alt_buttons) { // carrot : for CANIVAL 4TH HDA2
        ret = BUILD_SAFETY_CFG(hyundai_canfd_hda2_alt_buttons_rx_checks_scc2,
                               HYUNDAI_CANFD_HDA2_LONG_TX_MSGS);
      }
      else {
        ret = BUILD_SAFETY_CFG(hyundai_canfd_hda2_rx_checks_scc2,
                               HYUNDAI_CANFD_HDA2_LONG_TX_MSGS);
      }
    } else if (hyundai_canfd_hda2) {
      if (hyundai_canfd_alt_buttons) { // carrot : for CANIVAL 4TH HDA2
        ret = hyundai_canfd_hda2_alt_steering ? \
          BUILD_SAFETY_CFG(hyundai_canfd_hda2_alt_buttons_rx_checks,
                           HYUNDAI_CANFD_HDA2_ALT_STEERING_ALT_BUTTONS_TX_MSGS) : \
          BUILD_SAFETY_CFG(hyundai_canfd_hda2_alt_buttons_rx_checks,
                           HYUNDAI_CANFD_HDA2_ALT_BUTTONS_TX_MSGS);
      }
      else {
        ret = hyundai_canfd_hda2_alt_steering ? \
          BUILD_SAFETY_CFG(hyundai_canfd_hda2_rx_checks, HYUNDAI_CANFD_HDA2_ALT_STEERING_TX_MSGS) : \
      ...
      ret = hyundai_canfd_alt_buttons ? \
        BUILD_SAFETY_CFG(hyundai_canfd_radar_scc_alt_buttons_rx_checks,
                         HYUNDAI_CANFD_HDA1_STOCK_ALT_BUTTONS_TX_MSGS) : \
        BUILD_SAFETY_CFG(hyundai_canfd_radar_scc_rx_checks, HYUNDAI_CANFD_HDA1_STOCK_TX_MSGS);
    } else {
      // *** Non-HDA2 checks ***
