...
- CAN FD HDA1 hardware, PCM cruise, stock radar SCC, and the CRC-protected alternate `0x1AA` steering-wheel-button layout
- No openpilot longitudinal control and no camera SCC

At the instant of a physical stop, ordinary set-speed synchronization SET/RES traffic and automatic cruise-activation presses are blocked. Only a RES satisfying the departure-plan criterion below and the bounded keepalive remain eligible. Both RES paths must pass the current frame's pedal, Auto Hold, parking-brake, driver-button, CAN, and SCC fault/takeover interlocks. Ordinary speed-sync traffic has not been established as the cause of the reported on-car standstill failure.
...
The normal departure RES request requires engagement, an SCC-reported standstill, a nonempty speed plan, and **a finite end-of-horizon planned speed above `0.1 m/s`**. This DK KA4 scope restores the earlier final-speed criterion without additionally requiring `longitudinalPlan.shouldStop=false`. The near-term stopping threshold can otherwise classify a slow `0.3 m/s` departure plan as stopped with the default settings. A final planned speed of zero, at most `0.1 m/s`, or an invalid value does not request RES. Other vehicles and branches keep their existing resume criterion, and the keepalive schedule below is unchanged.
