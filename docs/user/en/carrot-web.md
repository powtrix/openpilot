...
### DK log-transfer scope and Carrot community data sharing

`System > Record & Power > DK Other Logs & Diagnostics Transfer` is the master consent for every manual or automatic external log and diagnostic transfer except separately consented KA4 validation to the fixed DK private NAS. It is off by default. Off blocks comma Athena, cloudlogs/statistics, remote log requests/SSH, Prime/Firehose, Sentry, the stock uploader, and user-started personal-NAS, Discord, dashcam, or tmux transfers. It closes live connections, aborts work in progress, and discards the previous consent session and Athena queue.

`Carrot Community Data Sharing` is also a high-risk consent that is off by default and can be enabled or used only while the master is on. Turning the master off clears community consent and shows the reason beside its disabled switch. With both enabled, Carrot community services may exchange device identifiers, vehicle name, branch/commit, local network address and heartbeat, all setting values and the catalog, automatic onroad/exception tmux diagnostics, setting snapshots, popular-setting values, CWP addresses, and bundled Discord notifications. Phone tethering may use mobile data, and disabling consent does not delete data already sent.

Carrot Web labels the current scope as `Only my DK NAS validation logs`, `All log transfers blocked`, or `Other logs and diagnostics allowed`. With the master off, `CarrotValidationAutoUpload` can still send only matching KA4 full rlogs to the fixed DK private NAS. Manual dashcam/tmux and user-configured NAS/Discord transfers require the master. The default `adot.synology.me` DK receiver remains automatic-validation-only and rejects manual transfers with HTTP 403, so manual upload requires a separately authenticated receiver. A remote-support tunnel itself is a separate non-log connection, but sending support/Vision diagnostics or Discord notices requires the master. None of the three consent values is stored in file backups, profiles, or QR transfer.
...
Ordinary post-drive onroad diagnostics and exception/CAN-error tmux diagnostics are captured and uploaded automatically only while both `DkThirdPartyDataSharing=1` and `CarrotCommunityDataSharing=1`. A CAN error is selected only when a currently received `carState` or `radarState` from the present onroad session reports it. A timeout left over from the previous drive is not used, and capture is delayed for five seconds after detection so the log includes the immediate aftermath. In an automatic diagnostic log, `CarrotException can_error queued` means the upload was queued; the preceding `current onroad CAN error detected from ...` line identifies the source used for the decision. Turning either consent off discards pending automatic exception transfers. A user-triggered `send tmux` also requires `DkThirdPartyDataSharing=1`, even for a configured NAS or custom webhook; Carrot Logs and bundled default Discord require community consent too.
...
On compatible eGPU model branches and hardware, an additional `eGPU big model` card appears. Downloading and verification may continue while driving, but request the compilation restart only when safely parked with ignition on. If compilation fails or the eGPU is unavailable, the device falls back to its internal model. The eGPU does not enable longitudinal control by itself, but the selected driving model's perception and path output can still affect lateral steering targets on a stock-longitudinal vehicle.
...
The remote-support tunnel itself is a user-started non-log connection and can still be created while community sharing is off. Sending its link/PIN or a Vision diagnostic bundle to any Discord destination requires `DkThirdPartyDataSharing=1`; bundled Carrot support Discord also requires community consent. Without consent, send the displayed details directly to the designated helper.
