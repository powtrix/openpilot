...
The `0915 부팅수정` release of `dkcarrot-wip` fixes the UI startup error introduced in `0913 감속`. It retains the automatic red bar and `감속 X.X m/s^2` (deceleration) at the bottom center of the driving screen. No setting is required. It does not change the 22 settings below or any acceleration, braking, or steering control, and it also works on vehicles using stock SCC.
...
The upper-left release label appears below the calendar date as `0915 부팅수정`, starting with the release date. It retains the 10% text-size increase introduced in `0913`. Existing date-display preferences remain unchanged: with the date hidden, the label appears below the clock or on its own at the upper left. It identifies the installed release, not the current date or the time the device finished updating.
...
Automatic CAN diagnostic logs are generated only while both `DkThirdPartyDataSharing=1` and `CarrotCommunityDataSharing=1`, and currently received vehicle or radar state from the present onroad session reports an actual error. CAN timeouts from the previous drive's shutdown are not used, and capture occurs five seconds after detection to include the immediate aftermath. The two sharing settings do not change speed, deceleration, or any vehicle-control decision. When either is off, or whenever no automatic diagnostic log was generated, explicitly select and upload the affected drive from Carrot Web for a speed/deceleration investigation.
