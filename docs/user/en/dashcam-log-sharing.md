...
- CAN FD HDA1 (not HDA2), stock radar SCC, and PCM cruise
- No openpilot longitudinal control and no camera SCC

During a drive it marks an engaged stock-SCC standstill after about `0.5 s`, including stops where the brake or Auto Hold state changes, so the start of the OEM state transition remains in the retained rlog. It does not press RES or alter a vehicle message. The old controller-request and OFF-state IDs are retained for compatibility with captures created by earlier builds. A physical RES press remains visible in the retained rlog. The collector also distinguishes lane-mode `PathOffset=0` from `PathOffset=10` (10 cm right) while `AdjustLaneOffset=0`. It selects sustained acceleration of at least `0.7 m/s²` for `0.5 s` while stock SCC is active, the driver is not pressing a pedal, and the car is closing on a nearby lead.
...
Each event capture sends **at most three full rlogs only**. A new campaign can collect each of five producible conditions up to twice, selecting **at most 10 captures / 30 distinct full rlogs**. Including compatibility records for two legacy conditions already retained by an older build, the state can handle at most **14 captures / 42 rlogs**. These are selected-item limits, not limits on transmission count or bytes. At most 5 captures / 750 MiB can wait locally at once, but the client has no cumulative transfer cap. The collector does not separately send `qcamera`, driver-camera, tmux data, or a Discord notification. A full rlog can nevertheless contain precise location, vehicle CAN and control state, device identifiers, branch, commit and working-tree modification status, Params captured at route start, and low-resolution road thumbnails sampled at roughly one-minute intervals. It does not photograph the Kia cluster or its exact alert text, so the text itself cannot be proven from an rlog alone.

For upload, the device signs a purpose-specific value bound only to that request's one-time challenge with its existing registration key. The receiver verifies the signature locally against a privately pre-enrolled public-key fingerprint; it neither receives a general comma API bearer nor contacts the official comma device API. The user does not enter a token or password. The automatic-validation destination is pinned to the root origin `https://adot.synology.me` on standard HTTPS port 443. User information, another port, a subpath, query or fragment, subdomains, lookalike domains, and arbitrary HTTPS origins are rejected; a deployment environment variable cannot override that pin. The ordinary Carrot Web upload destination cannot redirect it either. The receiver accepts only exact HDA1 KA4 metadata with no HDA2 flag and recomputes every file's size and SHA-256. The device removes a local queue entry only after the capture ID, verified device ID, complete file list, and final manifest hash all match the completion receipt. If the receiver does not support this authenticated protocol or identity verification is temporarily unavailable, the logs remain retained locally and retry automatically.

The queue survives a reboot. Failures retry after approximately 30 seconds, 2 minutes, 10 minutes, 1 hour, and 6 hours, with small timing jitter, then can continue about every 6 hours for up to seven days. A failed file PUT can retry immediately, and an authentication or completion-receipt failure can resend the entire capture. Collection is limited to two captures per condition, five concurrently pending captures overall, and about 750 MiB pending, but that is not a cumulative network-transfer cap. The receiver has a 1 GiB per-device daily limit, so retained local logs may retry automatically the next day after the limit is reached. The setting row shows only a sanitized state, pending count, expiry, and last-upload time; it never shows a route, receiver URL, device ID, capture ID, or raw error. The setting turns itself off after all three required conditions—an observed stock-SCC standstill, `PathOffset=10` lane operation, and stock-SCC acceleration while closing on a lead—upload, or after seven days. The old controller-request classification and `PathOffset=0` lane capture are optional diagnostics; the two legacy OFF IDs never count toward completion. Turning the setting off manually discards pending entries and releases retention markers created by this feature. Consent is excluded from settings backups, restores, and QR transfer; if the saved campaign state is unavailable after a reinstall, explicitly toggle the setting off and on again while parked.

Carrot Web has no general user login, but the activation token for this high-risk consent is issued only to a TCP connection from the phone providing the current Wi-Fi default gateway, or to a device-local connection, and the one-use token remains bound to that peer. Another LAN client on the same hotspot cannot enable this setting. Other Carrot Web features still trust the local network, so use it only on a private WPA2/WPA3 tether or hotspot with a strong unique password, not on public Wi-Fi. Automatic uploads over phone tethering can use mobile data; successive captures and retries can make total data use exceed the 750 MiB concurrent local queue limit. Disabling the setting does not delete data already stored on the server; ask the server administrator if deletion is required.
