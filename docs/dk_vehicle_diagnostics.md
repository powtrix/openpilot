...
`dkcarrot-wip`의 CI에는 빌드 이후 실제 전체 UI 시작 모듈을 불러오는 검사와 관련 회귀 테스트를 추가한다. `SCALE=1 BIG=1`로 자동 모니터 탐색을 막고, 격리된 CI 환경에서 실행한다. `main()`이나 차량 제어 프로세스는 시작하지 않는다. 푸시 전 주석 검사도 푸시 대상 커밋을 직접 읽어 실제 PyRay 생성 함수와 호환되지 않는 즉시 평가 주석을 거부한다. 주석 검사·모듈 로딩·화면 렌더링·실차 부팅은 서로 다른 검증 단계이며, 앞 단계 통과를 뒤 단계 통과로 보고하지 않는다.
