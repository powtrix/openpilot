...
설치본 루트의 `dk_release.json`은 `schema=1`, `deployed_at="YYYY-MM-DD HH:mm KST"`, 양의 정수 `diagnostics_version`, 릴리스 이름 `label`을 가진다. 배포 준비 시 저장소에서 갱신하는 릴리스 표식이며, 실제 GitHub 푸시 시간을 자동 조회하는 기능이 아니다. comma3x 주행 UI는 설치된 파일을 시작할 때 한 번 읽어 `dkcarrot-wip`에서만 좌상단 날짜 아래 `MMDD label`을 표시한다. `MMDD`는 항상 메타데이터의 배포 날짜에서 붙이며, 이번 배포는 `label="감속"`인 `0913 감속`이다. 글자 크기는 이전 48보다 10% 큰 52.8로, 날짜 글자 크기 60의 88%다. 기존 `DK 배포 YYYY-MM-DD` 문구는 중복 표시하지 않는다. 현재 날짜, Git 커밋 날짜, AGNOS 업데이트 날짜 또는 그 장치의 설치 완료 시각이 아니다.
