...
설치본 루트의 `dk_release.json`은 `schema=1`, `deployed_at="YYYY-MM-DD HH:mm KST"`, 양의 정수 `diagnostics_version`, 릴리스 이름 `label`을 가진다. 배포 준비 시 저장소에서 갱신하는 릴리스 표식이며, 실제 GitHub 푸시 시간을 자동 조회하는 기능이 아니다. comma3x 주행 UI는 설치된 파일을 시작할 때 한 번 읽어 `dkcarrot-wip`에서만 좌상단 날짜 아래 `MMDD label`을 표시한다. `MMDD`는 항상 메타데이터의 배포 날짜에서 붙이며, 이번 배포는 `label="부팅수정"`인 `0915 부팅수정`이다. 글자 크기는 이전 48보다 10% 큰 52.8로, 날짜 글자 크기 60의 88%다. 기존 `DK 배포 YYYY-MM-DD` 문구는 중복 표시하지 않는다. 현재 날짜, Git 커밋 날짜, AGNOS 업데이트 날짜 또는 그 장치의 설치 완료 시각이 아니다.

메타데이터가 없거나 형식이 잘못되면 임의 날짜를 대신 표시하지 않는다. `label`은 앞뒤 공백 없는 1~12자의 출력 가능한 문자열이어야 하며, 제어 문자·보이지 않는 형식 문자는 허용하지 않는다. 이전 메타데이터에 `label` 키가 없는 경우에만 `개선`을 사용한다. 별도 설정은 없으며 `ShowDateTime` 값을 강제로 바꾸지 않는다. 날짜를 숨기고 시간만 표시하면 시계 아래에, 시계·날짜를 모두 숨기면 좌상단에 릴리스 표식만 표시한다. 이 표시와 로컬 테스트 통과만으로 실차 문제 해결·디바이스 업데이트 완료를 판정하지 않는다.

## 0913 감속 표시 — 제어와 분리된 관측 UI

`dkcarrot-wip` 주행 화면 하단 중앙에 붉은 막대와 `감속 X.X m/s^2`를 표시한다. 글꼴 대체 경로에서도 단위가 깨지지 않도록 지수는 ASCII `^2`로 그린다. 별도 Params나 설정 없이 자동으로 표시하며, SCC·재출발·가감속·조향 제어와 CAN 출력을 바꾸지 않는다. 릴리스 표식이 유효한지와 감속 표시의 사용 여부는 독립적이다.

KA4의 입력 `carState.aEgo`는 바퀴 속도를 필터링해 추정한 가속도다. 음수일 때 그 절댓값을 감속량으로 표시한다. 브레이크 압력, 페달 밟은 비율 또는 `carControl.actuators.accel`의 감속 명령이 아니다. 순정 SCC 차량에서도 실제로 느려지는 정도를 볼 수 있지만, 순정 SCC 제동·운전자 제동·엔진 브레이크·경사 등의 원인을 구분하지 않는다. 막대는 `4.0 m/s²`에서 최대 길이가 되고 더 큰 값도 숫자는 그대로 표시한다. 최대 길이는 표시 눈금일 뿐 제동 한계나 안전 판정이 아니다.

표시는 현재 주행에서 새로 수신한 유효한 차량 상태와 경고 상태만 사용한다. 이동 속도 `0.5 m/s` 이상, 감속량 `0.1 m/s²` 이상일 때 표시하고, 정차·가속·작은 감속·수신 지연·유효하지 않은 값에서는 숨긴다. `abs(aEgo) > 20 m/s²`도 표시 입력으로 부적절한 표본으로 취급해 숨기며 제어 판단에는 사용하지 않는다. `carState`와 `selfdriveState` 모두 `alive`·`valid`이고 현재 주행의 `started_frame` 이후에 수신된 표본이어야 한다. 두 서비스의 수신 시각과 원본 `logMonoTime`을 각각 확인하여 `0.5초`를 넘는 과거 값은 재사용하지 않는다. 경고가 있거나 경고 입력을 신뢰할 수 없을 때도 표시하지 않아 경고가 우선한다. 표시가 없다는 것은 브레이크가 작동하지 않는다는 뜻이 아니다.

좁은 화면이나 다른 안내와 겹칠 때는 감속 표시를 숨긴다. 주행 화면 폭이 1,400 미만이거나 높이가 400 미만이면 숨기고, 내비 안내 영역과 겹칠 수 있는 구성에서는 폭 2,000 미만일 때 숨긴다. 모델 렌더러가 이미 그린 선행차·경로 끝 표시의 영역도 읽어 감속 문자나 막대와 겹치면 감속 표시를 생략한다. 기존 안내와 경고는 숨기거나 이동하지 않는다. 이는 UI 충돌 방지 조건이며 차량 제어 조건이 아니다.

## 0915 부팅수정 — 실제 PyRay 시작 검사

`95ca541582`에서 추가한 `ModelRenderer.deceleration_exclusion_rect`의 반환형 `rl.Rectangle | None`은 UI 모듈을 읽는 즉시 평가됐다. 실제 PyRay의 `Rectangle`은 클래스가 아니라 생성 함수이므로 `TypeError: unsupported operand type(s) for |: 'function' and 'NoneType'`으로 시작이 중단된다. 모델 모듈에 `from __future__ import annotations`를 추가해 주석의 즉시 평가를 없앴다. 제어·CAN·감속량 계산·기존 표시 순서는 바꾸지 않는다.

기존 단위 테스트는 Rectangle을 dataclass로 대체해 이 오류를 놓쳤다. 재현 검사에서는 실제 PyRay와 전체 모델 모듈을 별도 프로세스에서 읽고, 로컬 디바이스 자원에 의존하는 `ui_state` 싱글턴만 대체한다. 이전 `22e0daa1a8`은 로딩 성공, `95ca541582`는 위 오류, 수정본은 로딩 성공을 확인했다. 이 로컬 재현은 실제 장치에서 수정본이 부팅됐다는 증거가 아니다.

`dkcarrot-wip`의 CI에는 빌드 이후 실제 전체 UI 시작 모듈을 불러오는 검사와 관련 회귀 테스트를 추가한다. `SCALE=1 BIG=1`로 자동 모니터 탐색을 막고, 격리된 CI 환경에서 `OpenpilotPrefix()`로 실제 msgq 통신 디렉터리를 준비한 후 실행한다. 환경변수 `OPENPILOT_PREFIX`만 설정하면 디렉터리가 생성되지 않아 UI 구독 초기화가 실패하므로 그것만으로 준비 완료로 간주하지 않는다. `main()`이나 차량 제어 프로세스는 시작하지 않는다. 푸시 전 주석 검사도 푸시 대상 커밋을 직접 읽어 실제 PyRay 생성 함수와 호환되지 않는 즉시 평가 주석을 거부한다. 주석 검사·모듈 로딩·화면 렌더링·실차 부팅은 서로 다른 검증 단계이며, 앞 단계 통과를 뒤 단계 통과로 보고하지 않는다.
